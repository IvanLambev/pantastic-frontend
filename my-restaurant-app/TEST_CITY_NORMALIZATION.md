...
userCities.forEach(userCity => {
  ...
  const match = mockRestaurants.find(r => normalizeCityName(r.city) === normalizedUser);
...
console.log('Do they match?', normalizeCityName(userCity) === normalizeCityName(restaurantCity));
```
